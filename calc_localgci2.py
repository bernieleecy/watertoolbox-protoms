...
def ParseOptions():
    ...
    mode.add_argument('-b','--boxes',nargs="+",help="the pdb files of the box(es) within which you wish to analysis the water content,default='gcmc_box.pdb'",default=None)
    mode.add_argument('-c','--clusters',help="A cluster file that will be used to determine the center of the hydration spheres",default=None)
...
if __name__ == "__main__":
    ...
    if args.input == None:
        if args.clusters != None:
            (B, N) = read_gcmc(num_inputs,args.directories,args.resultsfiles,args.pdbfiles,args.residue,args.atom,args.skip,mode,structures=sites_centers,radius=args.radius)
        elif args.boxes != None and args.clusters == None:
            (B, N) = read_gcmc(num_inputs,args.directories,args.resultsfiles,args.pdbfiles,args.residue,args.atom,args.skip,mode,structures=boxes)
        data = {"Adams":B,"N":N}
